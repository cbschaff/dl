...
def make_atari_env(game_name, seed=0, rank=0, sticky_actions=True, timelimit=True, noop=False, frameskip=4, episode_life=True, clip_rewards=True, frame_stack=1, scale=False):
...
if __name__ == '__main__':
    ...
    class TestEnvFns(unittest.TestCase):
        def test_atari(self):
            ...
            assert 'episode_info' in env.step(env.action_space.sample())[3]
            env.close()
            env = make_atari_env('Breakout', 0, 0, sticky_actions=False, frame_stack=4)
            assert env.spec.id == 'BreakoutNoFrameskip-v4'
