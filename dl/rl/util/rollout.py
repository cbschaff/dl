...
class RolloutStorage(object):
    ...
    def __init__(self, num_steps, num_processes, device='cpu', other_keys=[],
        ...
        self.keys = (['ob', 'ac', 'r', 'mask', 'vpred'] + list(other_keys)
                     + list(recurrent_state_keys))
        self.data = None
        self.device = device
        self.step = 0
    ...
    def compute_targets(self, next_value, next_mask, gamma, use_gae=True,
        ...
        if use_gae:
            ...
            self.data['vtarg'][-1] = self.data['r'][-1] + (gamma * next_mask
                                                           * next_value)
            for step in reversed(range(self.num_steps - 1)):
    ...
    def compute_ob_stats(self):
        ...
        batch_var = self.data['ob'].view(
            -1, *self.data['ob'].shape[2:]).var(0, unbiased=False)
    ...
    class TestRollout(unittest.TestCase):
        ...
        def test_feed_forward(self):
            ...
            for batch in r.feed_forward_generator(6):
                assert (batch['ob'].shape == (6, 1, 84, 84)
                        or batch['ob'].shape == (2, 1, 84, 84))
            try:
