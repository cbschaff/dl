"""Extends Trainer with dataset iterators."""
import gin
import torch
...
class DatasetTrainer(Trainer):
    ...
    def __init__(self,
        ...
        self.dtrain = DataLoader(dataset_train, sampler=self._sampler,
                                 batch_size=batch_size, num_workers=num_workers)
        if dataset_val is None:
            self.dval = None
        else:
            self.dval = DataLoader(dataset_val, batch_size=batch_size,
                                   num_workers=num_workers, shuffle=False)
        self._diter = None
        self.nsamples = 0

    def _save(self, state_dict):
        assert '_sampler' not in state_dict, (
               "'_sampler' key is used to save epoch progress. "
               "Please change your key.")
        assert '_nsamples' not in state_dict, (
               "'_nsamples' key is used to "
               "save epoch progress. Please change your key.")
    ...
    def step(self):
        """Get next batch and calls update."""
        ...
        self.nsamples += min(self._dsize - (self.nsamples % self._dsize),
                             self.batch_size)

    def batch_to_device(self, batch):
        """Places a batch on the same device as the model."""
    ...
    def before_epoch(self):
        """Call before the start of each epoch."""
        pass
...
if __name__ == '__main__':
    ...
    class TestDatasetTrainer(unittest.TestCase):
        ...
        def test(self):
            ...
            trainer = T('test', dtrain, dval, 1, shuffle=False, eval=True,
                        eval_period=1, maxt=10)
            trainer.train()
            trainer = T('test', dtrain, dval, 1, shuffle=False, eval=True,
                        eval_period=1, maxt=20)
            trainer.train()
